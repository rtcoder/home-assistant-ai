...
# Skrypt do instalacji serwera Home Voice AI jako usługi systemd (Linux)

if [[ $EUID -ne 0 ]]; then
   echo "Błąd: Ten skrypt musi być uruchomiony z uprawnieniami roota (sudo)."
   echo "Użyj: sudo ./setup-service.sh"
   exit 1
fi

PROJECT_ROOT=$(pwd)
SERVICE_NAME="home-voice-ai-server"
...
[Unit]
Description=Home Voice AI Server Service
